...
app.get('/api/available-years/location', async (req, res) => {
  try {
    ...
    const tables = ['provinsi', 'kabupaten', 'kecamatan', 'kelurahan', 'das'];
    
    for (const table of tables) {
      try {
        const query = await db.query(
          `SELECT DISTINCT tahun_data FROM ${table} WHERE tahun_data IS NOT NULL ORDER BY tahun_data DESC`
        );
        result[table] = query.rows.map(r => r.tahun_data);
      } catch (err) {
        console.log(`Table ${table} might not exist or has no tahun_data`);
        result[table] = [];
      }
    }
...
app.get('/api/available-years/disaster/:tableName', async (req, res) => {
  ...
  try {
    ...
    res.json(result.rows.map(r => r.tahun_data));
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
app.get('/api/locations/:level', async (req, res) => {
  ...
  try {
    const tableConfig = {
      'provinsi': { table: 'provinsi', column: 'nama' },
      'kabupaten': { table: 'kabupaten', column: 'kab_kota' },
      'kecamatan': { table: 'kecamatan', column: 'kecamatan' },
      'kelurahan': { table: 'kelurahan', column: 'kelurahan' },
    ...
    if (filter_column && filter_value) {
      ...
      `;
      const mappingResult = await db.query(mappingQuery, [config.table, filter_column, filter_value]);
...
app.get('/api/locations/:level/max-year', async (req, res) => {
  ...
  try {
    ...
    let query = `
      SELECT DISTINCT ON (${config.column}) 
        ${config.column} as name,
        tahun_data
      FROM ${config.table}
      WHERE tahun_data <= $1
    `;
    ...
    if (filter_column && filter_value) {
      ...
      if (mappingResult.rows.length > 0) {
        const targetValue = mappingResult.rows[0].target_value;
        params.push(targetValue);
        query += ` AND UPPER(${filter_column}) = UPPER($${params.length})`;
      } else {
        params.push(filter_value);
        query += ` AND UPPER(${filter_column}) = UPPER($${params.length})`;
      }
    ...
    query += ` ORDER BY ${config.column}, tahun_data DESC`;
    ...
    console.error('Error fetching locations with max year:', error);
    res.status(500).json({ error: error.message });
  }
});
...
app.get('/api/locations/cascade/:level', async (req, res) => {
  ...
  try {
    ...
    if (level === 'kabupaten' && provinsi) {
      ...
      const mappingQuery = `
        ...
        WHERE source_table = 'provinsi'
          AND source_column = 'nama'
          AND target_table = 'kabupaten'
          AND target_column = 'provinsi'
          AND UPPER(source_value) = UPPER($1)
      `;
      const mappingResult = await db.query(mappingQuery, [provinsi]);
      ...
      // Cari mapping dari kabupaten ke kecamatan
      const mappingQuery = `
        SELECT DISTINCT target_value 
        FROM reference_mapping 
        WHERE source_table = 'kabupaten'
          AND source_column = 'kab_kota'
          AND target_table = 'kecamatan'
          AND target_column = 'kab_kota'
          AND UPPER(source_value) = UPPER($1)
      `;
      const mappingResult = await db.query(mappingQuery, [kabupaten]);
    ...
    console.error('Error in cascading locations:', error);
    res.status(500).json({ error: error.message });
  }
});
        ...
        try {
          ...
          for (const [dbCol, mapConfig] of Object.entries(mapping)) {
            ...
            if (mapConfig.type === 'shp_column') {
              ...
              if (mapConfig.data && mapConfig.data[insertedCount]) {
                values.push(mapConfig.data[insertedCount][dbCol]);
              } else {
                values.push(null);
              }
              placeholders.push(`$${paramIndex++}`);
...
app.delete('/api/layers/:tableName/year/:year', async (req, res) => {
  try {
    const { tableName, year } = req.params;
